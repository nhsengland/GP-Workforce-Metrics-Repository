...
metric_id_prefix = "DPC_FTE_NWRS_Net_of_Baseline"
...
@transform(
    output_metrics=Output("ri.foundry.main.dataset.d6714d3d-952c-4ff4-90be-3caf7967aeff",
                          checks=utils.get_metrics_dataset_checks("all")
                          ),
    output_metadata=Output("ri.foundry.main.dataset.918c6de9-88fb-4a26-990a-4de3402be5fe",
...
def perform_calculation(df_dpc, all_locations, ccg, spark_session):
    ...
    df_dpc = df_dpc.withColumn("dimension_1", f.lit("DPC NWRS Net of Baseline"))
    df_dpc.persist()
    df_dpc.count()
    ...
    metric_table = dpc_utils.generate_target(metric_table, "DPC NWRS Net of Baseline")
...
def perform_aggregation(df, dimensions):
    ...
    *args*:
    ...
    metrics_calc = f.sum("all_nwrs_net_of_baseline").cast("double")
...
def getMetadata(gp_df, cols, totals_dimension):
    return utils.generate_metadata(
        ...
        metric_title='Direct Patient Care (DPC) full time equivalent (FTE) national '
                     'workforce reporting service (NWRS) net of baseline',
        totals_dimension=totals_dimension,
        metric_description="Full time equivalent (FTE) Direct Patient Care (DPC) roles "
                           "in General Practice and PCNs from national workforce reporting "
                           "service (NWRS) net of March 2019 baseline",
        short_name="NWRS Net of Baseline",
