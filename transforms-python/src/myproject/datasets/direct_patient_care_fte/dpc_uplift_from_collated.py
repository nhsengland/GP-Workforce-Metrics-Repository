...
metric_id_prefix = "DPC_FTE_Uplift_from_Collated"
...
@transform(
    output_metrics=Output("ri.foundry.main.dataset.2d6d60f9-f26e-433e-8ec4-d79193bf0b1f",
                          checks=utils.get_metrics_dataset_checks("all")
                          ),
    output_metadata=Output("ri.foundry.main.dataset.5185ea32-eba5-4d78-a131-8ae89b7de6ed",
...
def perform_calculation(df_dpc, all_locations, ccg, spark_session):
    ...
    df_dpc = df_dpc.withColumn("dimension_1", f.lit("DPC Uplift from Collated")) \
        .withColumn("uplift", f.col("collated_figure_absolute") - f.col("all_nwrs_absolute"))
    df_dpc.persist()
    df_dpc.count()
    ...
    metric_table = without_baseline.join(f.broadcast(dpc_metadata.select('metric_id', 'dimension_1', 'dimension_2',
    ...
    metric_table = dpc_utils.generate_target(metric_table, "DPC Uplift from Collated")
...
def perform_aggregation(df, dimensions):
    ...
    *args*:
    ...
    metrics_calc = f.sum("uplift").cast("double")
...
def getMetadata(gp_df, cols, totals_dimension):
    return utils.generate_metadata(
        ...
        metric_title='Direct Patient Care (DPC) full time equivalent (FTE) uplift from '
                     'collated figures',
        totals_dimension=totals_dimension,
        metric_description="Difference between number of full time equivalent (FTE) "
                           "Direct Patient Care (DPC)  roles in General Practice and PCNs "
                           "from collated figures and number of FTE DPC roles from NWRS",
        short_name="Uplift from Collated",
